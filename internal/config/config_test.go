...
// TestLoadAppConfig_GetLogsMaxConcurrent pins the process-wide eth_getLogs
// bound — the knob that actually enforces the provider rate limit. Per-scan
// knobs cannot: jobs.token_worker.concurrency scans run at once, each fanning
// out into scan_window_concurrency windows × 3 merged queries, so the aggregate
// at defaults (5 × 2 × 3 = 30) would exceed Infura's free-tier ~10 req/s no
// matter how the per-scan factors are tuned. The aggregate-default invariant is
// therefore pinned against THIS knob, which caps the aggregate directly.
func TestLoadAppConfig_GetLogsMaxConcurrent(t *testing.T) {
	setMinimalRequiredEnv := func(t *testing.T) {
		t.Helper()
		t.Setenv("FF_INDEXER_DATABASE_HOST", "localhost")
		t.Setenv("FF_INDEXER_DATABASE_DBNAME", "ff")
	...
	t.Run("default caps the aggregate under the free-tier rate limit", func(t *testing.T) {
		setMinimalRequiredEnv(t)
		cfg, err := LoadAppConfig("", "")
		require.NoError(t, err)
		require.Equal(t, 8, cfg.Ethereum.GetLogsMaxConcurrent)

		const infuraFreeTierReqPerSec = 10
		require.LessOrEqual(t, cfg.Ethereum.GetLogsMaxConcurrent, infuraFreeTierReqPerSec,
			"default process-wide eth_getLogs concurrency must not exceed the free-tier rate limit")

		// Document WHY the process-wide cap is the load-bearing one: the
		// uncapped aggregate of the per-scan knobs at defaults is well above it.
		const mergedQueriesPerWindow = 3
		uncappedAggregate := cfg.Jobs.TokenWorker.Concurrency * cfg.Ethereum.ScanWindowConcurrency * mergedQueriesPerWindow
		require.Greater(t, uncappedAggregate, infuraFreeTierReqPerSec,
			"sanity: without the process-wide cap the default aggregate would exceed the limit — if this ever flips, the cap is no longer what protects the deployment")
		require.Less(t, cfg.Ethereum.GetLogsMaxConcurrent, uncappedAggregate,
			"the cap must actually bind at defaults")
	})

	t.Run("env override is honored", func(t *testing.T) {
		setMinimalRequiredEnv(t)
		t.Setenv("FF_INDEXER_ETHEREUM_GETLOGS_MAX_CONCURRENT", "32")
		cfg, err := LoadAppConfig("", "")
		require.NoError(t, err)
		require.Equal(t, 32, cfg.Ethereum.GetLogsMaxConcurrent)
	})

	t.Run("zero means unbounded and is accepted", func(t *testing.T) {
		setMinimalRequiredEnv(t)
		t.Setenv("FF_INDEXER_ETHEREUM_GETLOGS_MAX_CONCURRENT", "0")
		cfg, err := LoadAppConfig("", "")
		require.NoError(t, err)
		require.Equal(t, 0, cfg.Ethereum.GetLogsMaxConcurrent)
	})

	t.Run("negative rejected at startup", func(t *testing.T) {
		setMinimalRequiredEnv(t)
		t.Setenv("FF_INDEXER_ETHEREUM_GETLOGS_MAX_CONCURRENT", "-1")
		_, err := LoadAppConfig("", "")
		require.Error(t, err)
		require.Contains(t, err.Error(), "getlogs_max_concurrent must be >= 0")
	})
}
