...
// TestUpdateTokenSpamStatus verifies spam verdict updates and token event broadcasts
func TestUpdateTokenSpamStatus(t *testing.T) {
	if testDB == nil {
		t.Skip("Test database not initialized")
	}
	...
	// Create a test token
	token := &schema.Token{
		TokenCID: "bafyreihxq5urqkblomcf4yvexqzaqqjwvvuihmefj4wktkxw3pnykkhju",
		IsSpam:   false,
	}
	err := testDB.WithContext(ctx).Create(token).Error
	require.NoError(t, err)

	t.Run("updates token spam status when changed", func(t *testing.T) {
		changed, err := store.UpdateTokenSpamStatus(ctx, token.ID, true)
		require.NoError(t, err)
		assert.True(t, changed)

		// Verify the token is updated
		var updated schema.Token
		err = testDB.WithContext(ctx).First(&updated, token.ID).Error
		require.NoError(t, err)
		assert.True(t, updated.IsSpam)

		// Verify a token event was created
		var event schema.TokenEvent
		err = testDB.WithContext(ctx).
			Where("token_id = ? AND event_type = ?", token.ID, schema.EventTypeSpamStatusChanged).
			First(&event).Error
		require.NoError(t, err)
		assert.Equal(t, token.ID, event.TokenID)
		assert.Nil(t, event.OwnerAddress) // Broadcast to all owners
	})

	t.Run("returns false when spam status unchanged", func(t *testing.T) {
		...
		require.NoError(t, err)
		// Should still be 1 from the first test
		assert.Equal(t, int64(1), eventCount)
	})

	t.Run("clears spam status and creates event", func(t *testing.T) {
		changed, err := store.UpdateTokenSpamStatus(ctx, token.ID, false)
		require.NoError(t, err)
		assert.True(t, changed)

		// Verify the token spam status is cleared
		var updated schema.Token
		err = testDB.WithContext(ctx).First(&updated, token.ID).Error
		require.NoError(t, err)
	...
	// Cleanup
	t.Cleanup(func() {
		testDB.Exec("DELETE FROM token_events WHERE token_id = ?", token.ID)
		testDB.Exec("DELETE FROM tokens WHERE id = ?", token.ID)
	})
}
