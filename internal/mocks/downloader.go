...
//
//	mockgen -source=downloader.go -destination=../mocks/downloader.go -package=mocks -mock_names=Downloader=MockDownloader
//
...
func (m *MockDownloader) Download(ctx context.Context, url string) (*downloader.DownloadResult, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "Download", ctx, url)
	ret0, _ := ret[0].(*downloader.DownloadResult)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}
