...
// TestOwnerScanDiscovery_Integration validates the two-step owner scan
// (FetchOwnerLogsWindow + DiscoverOwnedTokensFromLogs) against live chain data.
// Discovery is always full since the scan-session redesign, so only full-range
// golden cases remain (the former limit-truncation cases are obsolete).
func TestOwnerScanDiscovery_Integration(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			logs, err := client.FetchOwnerLogsWindow(ctx, tc.owner, tc.fromBlock, tc.toBlock)
			require.NoError(t, err)
			got, err := client.DiscoverOwnedTokensFromLogs(ctx, tc.owner, logs, nil)
			require.NoError(t, err)
