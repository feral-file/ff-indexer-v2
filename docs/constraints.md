...
- **Outbound URL fetch (media)** — The media health sweeper and media worker (when enabled) perform HTTP(S) fetches for **stored media and source URLs** that may originate from chain metadata. By default those clients apply **SSRF controls** (blocked RFC-private/link-local/loopback/documentation ranges, cloud-metadata hostnames, redirect limits). **`security.ssrf_protection.allowlist.domains`** skips hostname/DNS/IP validation for matching hosts (including subdomains); each domain entry must contain **at least one dot** so bare public suffixes cannot be allowlisted. **`allowlist.ips`** only overrides IP-range rules (IPv4-mapped literals match listed IPv4 addresses). Treat allowlists like firewall exceptions—minimal entries with explicit operator review. **DNS resolution failures** are classified separately from SSRF blocks so health checks can retry without marking `SSRFBlocked`.
