...
- **Media health scope** — **`healthy`** means a validated probe succeeded **at the moment of the sweep**, from **this service's network position**. Content validation makes it a statement about the bytes served, not merely a 2xx; it is still **not** a promise that a viewer's next request will succeed. An origin that fails **intermittently** can pass a sweep and fail the request a player makes seconds later — health samples, it does not subscribe. A healthy verdict may also describe a **promoted fallback gateway** rather than the URL originally stored. Treat green health as “this reference served valid bytes when last checked”, not as a playback SLA: investigate playback reports even when health is green, and do not use `healthy` alone to rule out an unreliable upstream gateway. Turning sampling into a reliability signal (for example, tracking failure rates across sweeps rather than a single pass/fail) would change `MediaHealthStatus` semantics, `viewable` computation, and webhook payloads, so it requires its own spec. See [`#76`](https://github.com/feral-file/ff-indexer-v2/issues/76).
