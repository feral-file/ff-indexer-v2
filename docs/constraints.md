...
- **Chain ingestion block cursor** — The durable checkpoint (per-chain keys in `key_value_store`) advances only after a successful block-boundary flush and does **not** move backward because of late-arriving events. Ethereum `start_block` and Tezos `start_level` (YAML) **override** subscription start **regardless** of the stored cursor when non-zero; replays from below the current checkpoint are skipped by the runner until the stream is current. **Intentional rewind or reindex** from an earlier height requires operators to **reset or lower** the stored cursor (and set the desired start) so subscription, checkpoint, and ops intent stay aligned. **Known loss surfaces** (shutdown drop of the open block, timeout flush then restart on live-only Tezos, Tezos buffer overflow) are documented under *Accepted durability gaps* in [`docs/architecture.md`](architecture.md#accepted-durability-gaps-rare--edge-triggered). Details: [`docs/architecture.md`](architecture.md#chain-ingestion).

- **Tezos TzKT WebSocket resume** — TzKT SignalR subscriptions are **live-only** (hub methods do not accept a starting level). The runner still passes **`fromLevel`** for consistency with other chains, but Tezos does not yet **REST-backfill** before the socket. After **long downtime**, a **level gap** may exist until `TODO(tezos-resume)` or manual/API-driven recovery. See [`docs/architecture.md`](architecture.md#tzkt-websocket-and-resume-gap-crash-recovery).
