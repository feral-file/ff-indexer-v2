...
- **Media health scope** — **`healthy`** means the media reference answered successfully on **at least one** attempt during a sweep, from **this service's network position**. It does **not** promise that a player's **next single request** will succeed. The checker spends up to three attempts per verdict (HEAD, then GET with `Range`, then plain GET) and may additionally probe configured IPFS/Arweave/OnChFS gateways, while a browser or FF1 device issues **one** request. Against an intermittently failing origin the two disagree by construction: a URL that fails roughly a third of single requests is still reported **healthy** on most sweeps. Treat green health as “the resource exists and is reachable”, not as a playback SLA — investigate playback reports even when health is green, and do not use `healthy` alone to rule out an unreliable upstream gateway. Narrowing this gap (for example, surfacing per-attempt flakiness rather than a single pass/fail) would change `MediaHealthStatus` semantics, `viewable` computation, and webhook payloads, so it requires its own spec. See [`#76`](https://github.com/feral-file/ff-indexer-v2/issues/76).
