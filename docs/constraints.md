...
- **Chain ingestion block cursor** — The durable checkpoint (per-chain keys in `key_value_store`) advances only after a successful block-boundary flush and does **not** move backward because of late-arriving events. Ethereum `start_block` and Tezos `start_level` (YAML) **override** subscription start **regardless** of the stored cursor when non-zero; replays from below the current checkpoint are skipped by the runner until the stream is current. **Intentional rewind or reindex** from an earlier height requires operators to **reset or lower** the stored cursor (and set the desired start) so subscription, checkpoint, and ops intent stay aligned. Details: [`docs/architecture.md`](architecture.md#chain-ingestion).
