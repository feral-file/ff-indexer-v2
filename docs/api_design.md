...
- **Polling caveat:** when polling `IndexRelease` progress, pass **both** `include_unviewable=true` and `include_moderated=true`. objkt is a supported release-indexing vendor and also publishes a moderation signal, so a banned mint would never appear under the defaults and the documented termination condition would never fire.
- **`include_moderated`:** Default **`false`** — moderated tokens (OpenSea `is_disabled`, objkt takedown) are excluded from results. This is a **compatibility** change: existing clients that sent no new parameter will stop seeing them, which is the intent (every display surface, including FF1 dynamic playlists, inherits the filter). Pass `include_moderated=true` to see them, e.g. for a "filtered items" view. The `moderation_status` field (`none` | `spam`, a string enum so new verdict kinds do not change its type) is present on every token response.
- **`include_moderated` on the detail lookup:** `GET /api/v1/tokens/{cid}` and GraphQL `token(cid:)` take the same parameter with the same `false` default. A detail lookup is still a render path — display surfaces resolve a token by CID before showing it — so filtering only the list would leave flagged tokens fully renderable. A filtered token responds **`404`** (GraphQL: not-found error), deliberately indistinguishable from an unknown CID so the endpoint cannot be used to enumerate what is flagged. Release membership below now follows the same default.
...
- **Membership completeness (viewability only):** All members are returned regardless of `is_viewable` so the list is stable across viewability changes — a token may be temporarily unviewable while its media is processing, and membership is a data relationship, not a render gate. Callers needing only publicly visible members can still use `GET /api/v1/tokens?release_id=...` with `include_unviewable=false`.
- **`include_moderated` on release membership:** Default **`false`**, the same as every other token-returning path; pass `include_moderated=true` to include moderated members. The moderation verdict deliberately does **not** get the viewability exception above: unviewability is a transient pipeline state, whereas a moderation verdict is a decision about the content, and `members` is a full `TokenList` a client can render straight from — leaving it unfiltered would be one more route for a moderated token to reach a display surface. This is a **compatibility** change for any client reading `members` and expecting every mint to be present; `total_mints` on the release still reports the declared edition size, so a gap between `total_mints` and the member count now means "some members are filtered", not "indexing incomplete". Polling `IndexRelease` progress is unaffected — that flow uses `GET /api/v1/tokens` / `tokens(...)` with explicit `include_moderated=true`, not this endpoint.
...
- **Event types are open:** clients must ignore `event_type` values they do not recognize rather than failing the sync, so new types can be added without breaking existing consumers.
- **`moderation_status_changed`:** broadcast (`owner_address` is null) when a token's combined moderation status flips. Its metadata carries **`token_cid`** alongside `moderation_status` (the new status string), unlike `viewability_changed`, because the event envelope only identifies the token numerically and resolving that id back to a CID would require a `tokens(...)` lookup that the moderation filter excludes by default.
- **An event does not guarantee the token is fetchable.** `moderation_status` lives on the shared token row, so for a multi-owner airdrop the verdict is usually decided by the first owner's enrichment. Every wallet that acquires the token afterwards receives only an `acquired` event and no `moderation_status_changed`, because the verdict never changes again — yet `GET /api/v1/tokens` will not return that token under the defaults. Clients should treat an empty lookup for an announced token as "filtered", not as an error.
