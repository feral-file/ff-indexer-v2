...
- **Polling caveat:** when polling `IndexRelease` progress, pass **both** `include_unviewable=true` and `include_spam=true`. objkt is a supported release-indexing vendor and also publishes a moderation signal, so a banned mint would never appear under the defaults and the documented termination condition would never fire.
- **`include_spam`:** Default **`false`** — vendor-flagged spam tokens (OpenSea `is_disabled`, objkt `flag=banned`) are excluded from results. This is a **compatibility** change: existing clients that sent no new parameter will stop seeing flagged tokens, which is the intent (every display surface, including FF1 dynamic playlists, inherits the filter). Pass `include_spam=true` to see flagged tokens, e.g. for a "filtered items" view. The `is_spam` field is present on every token response.
- **`include_spam` on the detail lookup:** `GET /api/v1/tokens/{cid}` and GraphQL `token(cid:)` take the same parameter with the same `false` default. A detail lookup is still a render path — display surfaces resolve a token by CID before showing it — so filtering only the list would leave flagged tokens fully renderable. A filtered token responds **`404`** (GraphQL: not-found error), deliberately indistinguishable from an unknown CID so the endpoint cannot be used to enumerate what is flagged. Release membership below now follows the same default.
...
- **`include_spam` on release membership:** Default **`false`**, the same as every other token-returning path; pass `include_spam=true` to include flagged members. The spam verdict deliberately does **not** get the viewability exception above: unviewability is a transient pipeline state, whereas a moderation verdict is a decision about the content, and `members` is a full `TokenList` a client can render straight from — leaving it unfiltered would be one more route for a flagged token to reach a display surface. This is a **compatibility** change for any client reading `members` and expecting every mint to be present; `total_mints` on the release still reports the declared edition size, so a gap between `total_mints` and the member count now means "some members are filtered", not "indexing incomplete". Polling `IndexRelease` progress is unaffected — that flow uses `GET /api/v1/tokens` / `tokens(...)` with explicit `include_spam=true`, not this endpoint.
...
- **`spam_status_changed`:** broadcast (`owner_address` is null) when a token's vendor moderation verdict flips. Its metadata carries **`token_cid`** alongside `is_spam`, unlike `viewability_changed`, because the event envelope only identifies the token numerically and resolving that id back to a CID would require a `tokens(...)` lookup that the spam filter excludes by default.
- **An event does not guarantee the token is fetchable.** `is_spam` lives on the shared token row, so for a multi-owner airdrop the verdict is usually decided by the first owner's enrichment. Every wallet that acquires the token afterwards receives only an `acquired` event and no `spam_status_changed`, because the verdict never changes again — yet `GET /api/v1/tokens` will not return that token under the defaults. Clients should treat an empty lookup for an announced token as "filtered", not as an error.
