...
- **Membership completeness:** All members are returned regardless of `is_viewable` **or `is_spam`** state so the list is stable across viewability and moderation changes (tokens may be temporarily unviewable during media processing; a vendor may flag or unflag an item at any time). Membership is a data relationship, not a visibility gate. Callers needing only publicly visible members should use `GET /api/v1/tokens?release_id=...` with the defaults `include_unviewable=false` and `include_spam=false`.
