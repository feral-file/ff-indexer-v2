...
| consecutive_failures | INT | Failed sweeper checks since last success (drives error backoff); reset on success |
...
- `stalled` - The page failed to load or screenshot within the timeout; recorded for telemetry, carries the counter and gate unchanged, never gates (migration 028 reset legacy counters)
