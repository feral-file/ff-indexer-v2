...
**Failure policy — fall through, never stall.** Any warehouse failure — a scope refusal (`-32000 out of warehouse scope: …`: the range is below its coverage, the filter names a signature it does not store, or it is under a backfill), a transport error, or the single-attempt `ethereum.log_warehouse_timeout` — hands the **whole** query to the vendor path, logged at WARN with the reason. There is no retry against the warehouse: an outage degrades to pre-warehouse cost, which the credit guards still bound, not to a stalled scan. The one exception is a canceled or expired caller context, which is returned as such. A warehouse `query returned more than N results` (its result cap, 100k) is a **split** signal, not a fall-through: the range is bisected until each half fits, with no sleeps. If the vendor leg then fails for any reason other than the walk's own deadline, the caller gets **no** logs rather than the warehouse-served prefix — a partial history must never look complete.
