...
A source that fetches blocks in ranges knows a block is complete without waiting for the next one's events. Ethereum's subscriber implements `blockchain.ProgressReporter`: after emitting **each 20-block batch** it reports the batch's upper bound through the **same ordered queue** as the events, and the report **returns only once the runner has applied it** — the open block (if at or below the reported height) is flushed and the cursor is persisted to that height **even when the batch carried no events**. "Reported" therefore means "durable": a failure in a later batch, and the restart it causes, resumes after the last reported batch, never re-scans it, and can never trip `max_catchup_blocks` on already-scanned ranges. The monotonic guard applies to markers too. Tezos does not report progress; its cursor still advances only on event-bearing flushes.
...
**Catch-up batches:** a gap is fetched in 20-block batches (~9.4k raw topic matches — under Infura's 10k-result cap and a few MB — because ERC-20 Transfers share the ERC-721 signature and are only discarded at parse time), each batch emitted before the next is fetched, so a week-long gap streams through memory instead of materializing ~23M logs.
