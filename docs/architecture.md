...
5. **Sweeper (media)** — Monitors media URL health and can enqueue jobs (e.g. webhook notify). A verdict comes from one validated ranged GET of the stored URL (retried unranged only when the `206` prefix is unusable, or on `416`); on failure, IPFS/Arweave/OnChFS URLs fall back to probing the configured gateways for the **same resource reference**, including any query parameters that identify the asset (OnChFS artwork iterations depend on them). Each verdict is therefore a **point-in-time sample**, and a healthy one may describe a promoted fallback gateway rather than the stored URL — see the media health scope constraint in `docs/constraints.md`. Media health HTTP checks apply **SSRF controls** by default. The **worker core** (token queue) and **media worker** use the same SSRF-protected HTTP client for outbound metadata/media fetches when `security.ssrf_protection.enabled` is true (media worker requires CGO when enabled; configure under `security.ssrf_protection`; see `docs/constraints.md`).
