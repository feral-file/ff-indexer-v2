...
**Reorgs / duplicate heads:** a head below `next` re-fetches from that height; the replaced blocks' events are re-emitted at the same heights, which the runner accepts (same-height flush) and job unique keys deduplicate — the same behavior the push stream had (its `removed: true` logs were ignored).
