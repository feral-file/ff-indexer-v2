...
1. **Shutdown without flushing the in-flight block** — On process stop, the runner **does not** flush the current partial block; the durable cursor stays at the last successful block-boundary flush. Events already accepted into RAM for the unfinished height are **not** replayed from TzKT on restart (REST backfill covers committed cursor+1 through head at subscribe; the open partial block is still lost). On Ethereum the open height **is** re-fetched on restart, because ingestion pulls `[cursor+1, head]` by block number (see *Ethereum ingestion specifics*); the replay lands on the same job unique keys.
...
Ethereum chain ingestion is **head-driven pull**, not a log push stream. The provider (`internal/providers/ethereum/subscriber.go`) subscribes to **`newHeads`** over WebSocket and, for each head `H`, fetches every indexable log in `[next, H]` with **one `eth_getLogs`** over the same connection (`FetchIngestionLogs`: the standard ERC-721/ERC-1155/EIP-4906 signatures plus the registry's custom signatures, walked under the configured span cap), emits them in `(block, log index)` order, and sets `next = H + 1`. Heads queued while a fetch runs are coalesced into the next range.
...
**Reorgs / duplicate heads:** replacement headers at already-emitted heights (geth emits every new canonical header, so several can share a height) re-fetch from the **lowest** such height through the newest head — coalescing keeps the lowest queued height, never just the newest. If a head continues numerically but its parent is not the last emitted head (a reorg without a replacement header we could act on), the subscriber re-fetches a **2-block overlap**. Continuity is checked against the **node-reported hash** carried by the `newHeads` notification (`adapter.BlockHead`), not a locally recomputed `Header.Hash()` — the latter does not match for current mainnet headers and would fire the overlap on every block. Two is the most the runner can use: it flushes block *N* when *N+1*'s first event arrives and accepts a same-height re-flush, so the open block and the one just flushed are recoverable, while anything older is behind the monotonic cursor and dropped no matter what is re-fetched — the same limit the push stream had (its `removed: true` logs were ignored). Re-emitted heights land on the runner's same-height tolerance and the job unique keys. A future `start_block` is a hard lower bound: heads below it are ignored and no re-fetch dips under it.
