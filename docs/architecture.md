...
**Owner scans** additionally plan their windows around the head (`docs/address_scan_sessions.md`, "Window size"): the warehouse-covered range is cut into `ethereum.log_warehouse_scan_window_blocks` windows (default 1M, so a full mainnet scan is ~26 windows per merged query), the residual keeps the vendor's cap-sized windows. The head is read once for planning; each window's fetch re-checks it, so a stale plan costs at most a larger vendor walk inside one window.
