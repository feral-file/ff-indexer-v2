...
**Resume:** `fromBlock` (persisted cursor + 1, or `start_block`) is honored for real. `eth_subscribe("logs")` never replayed history, so blocks mined during a socket drop were lost; now the first head after (re)subscribe fetches the whole gap in order before live blocks continue. **`ethereum.max_catchup_blocks`** (default 50,000 ≈ 7 days) bounds that gap: a wider one fails subscribe with `ErrCatchupTooLarge` instead of silently walking history from a stale or reset cursor — operators lower the gap deliberately (see [`docs/constraints.md`](constraints.md)). `0` disables the bound.
