...
A source that fetches blocks in ranges knows a block is complete without waiting for the next one's events. Ethereum's subscriber implements `blockchain.ProgressReporter`: after emitting a range it reports its upper bound through the **same ordered queue** as the events. On a marker the runner **flushes the open block** (if at or below the reported height) immediately and **persists the cursor to the reported height even when the range carried no events**, so a resume never re-scans empty blocks and can never trip `max_catchup_blocks` on already-scanned ranges. The monotonic guard applies to markers too. Tezos does not report progress; its cursor still advances only on event-bearing flushes.
