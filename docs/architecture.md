...
5. **Sweeper (media)** — Monitors media URL health and can enqueue jobs (e.g. webhook notify). For an HTTP(S) URL a verdict comes from one validated ranged GET of the stored URL (retried unranged only when the `206` prefix is unusable, or on `416`); `data:` URIs are decoded and validated in-process instead, with no request. On failure, IPFS/Arweave/OnChFS URLs fall back to probing the configured gateways, but **what the fallback preserves differs by scheme**: IPFS keeps the path after the CID and OnChFS keeps the full reference including the query parameters that select an artwork iteration, whereas the **Arweave** fallback probes the transaction ID alone — so a healthy fallback verdict for `.../<txID>/<path>` can describe the transaction root rather than that sub-path. The token-indexing path runs the same checker, so a stored verdict is not always the sweeper's. Each verdict is a **point-in-time sample**, and a healthy one may describe a promoted fallback gateway rather than the stored URL — see the media health scope constraint in `docs/constraints.md`. Media health HTTP checks apply **SSRF controls** by default. The **worker core** (token queue) and **media worker** use the same SSRF-protected HTTP client for outbound metadata/media fetches when `security.ssrf_protection.enabled` is true (media worker requires CGO when enabled; configure under `security.ssrf_protection`; see `docs/constraints.md`).
