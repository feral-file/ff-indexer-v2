...
| `ethereum.log_warehouse_vendor_fallthrough` | `false` (strict) | warehouse-outage policy: `false` fails the query with an ERROR and never touches the metered vendor (a warehouse outage can't silently burn vendor credits); `true` falls through to the vendor for the whole query at pre-warehouse cost, logged at WARN. The above-head split always reaches the vendor regardless |
| `ethereum.log_warehouse_timeout` | `120s` | per-request deadline, one attempt, no retry — on expiry the query fails (or falls through to the vendor when `log_warehouse_vendor_fallthrough` is set) |
...
Locally, `make quickstart` in the ff-eth-logs repo starts a **tail-only** warehouse on `http://localhost:8545` (it serves only the blocks it has followed since it started). The indexer **refuses** such a warehouse: on mainnet it runs three capability probes before routing anything — the CryptoPunks internal `Transfer` (block 3,919,706) and the `erc1155Id` filter's TransferSingle (block 14,045,001) and URI (block 6,938,761) arms (`docs/architecture.md`, "Nothing is routed through an unverified warehouse") — and a warehouse whose coverage starts above those blocks, or that predates the `erc1155Id` filter, fails at startup. **Deployment prerequisite:** the `erc1155Id` probes require [ff-eth-logs #8](https://github.com/feral-file/ff-eth-logs/pull/8); it must be deployed to the warehouse **before** an indexer image carrying these requirements. An answered-but-failed capability probe is **fatal at startup** (like the chain-id and CryptoPunks checks), so deploying this indexer against an un-upgraded warehouse stops the worker from starting — deploy order is load-bearing. (A merely unreachable warehouse is tolerated and re-verified on first use.) To exercise routing locally, load the backfill first (`make backfill` in ff-eth-logs). The routing itself is visible in the logs: `Log warehouse verified` (info), `Log range served by the warehouse` (debug), and — on a warehouse failure — either `log warehouse unavailable for range and vendor fall-through is disabled; failing the query` (error, the strict default; the stage, block range, and cause are structured fields, and the returned error wraps them) or `Log warehouse unavailable for range, falling through to the vendor` (warn, when `log_warehouse_vendor_fallthrough` is set). Sizing note for the warehouse side: each owner-scan window issues the three merged owner queries at once, so the warehouse sees up to `token_worker.concurrency × scan_window_concurrency × 3` concurrent queries against its PostgreSQL pool.
...
**Autocommit-only migrations:** most migrations are transactional, but files using `CONCURRENTLY` (index create/drop) or per-batch commits **must run outside a transaction** — apply them with a plain `psql -f` in autocommit, and if you use a runner that auto-wraps files in `BEGIN/COMMIT`, disable wrapping for these. Current examples: `017_dedup.sql`, `030_drop_unused_indexes.sql` (35 `DROP INDEX CONCURRENTLY IF EXISTS`, no lock, re-runnable) and `031.sql` (one `CREATE INDEX CONCURRENTLY`, no lock, re-runnable). See each file's header.
