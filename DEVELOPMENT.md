...
| Load behaviour (measured 2026-08-26, same script, sequential) | owner-shaped 10k-block `eth_getLogs`: idle 1.6–1.8 s; queues linearly with concurrency (p50 4 s at 32–64, 8 s at 128, ≈12 req/s) with an occasional `503 service temporarily unavailable`; unfiltered topic-only 10k scans stay ≤ 4 s even at 32 concurrent | owner-shaped: idle 0.2 s, flat p50 ≤ 0.4 s up to 64 concurrent; at 128 the gateway starts **holding connections** and closed one without a response after ~85 s (retryable as EOF) — a connection limit, not CPU. **Unfiltered topic-only 10k scans serialize** (~0.6 s each: p50 4 / 8 / 22 s at 8 / 16 / 32 concurrent) and stall trivial calls while they drain (`eth_blockNumber` up to 5.5 s, 17 s to recover); abandoned requests keep running server-side. Production never issues that shape (catch-up uses 10-block windows; adapter queries are address/owner-scoped) — keep it that way. Prod peak is `token_worker.concurrency × scan_window_concurrency × 3` = 360 in flight, above the 128 measured |
