...
- **Tezos long downtime**: On restart, the Tezos subscriber **REST-backfills** from the persisted cursor (`fromLevel`) through chain head before opening the TzKT WebSocket. Logs include **“TzKT subscribe starting with REST backfill before SignalR”** and **“TzKT REST backfill complete, connecting SignalR”**. If backfill fails, subscription aborts and the cursor stays at the last committed level. Remaining edge cases (open partial block on shutdown, timeout flush, buffer overflow) are documented in [`docs/architecture.md`](docs/architecture.md#accepted-durability-gaps-rare--edge-triggered).
